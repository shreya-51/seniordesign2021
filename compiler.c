...
float execute_instruction(){
    ...
    if(head->operation == 1){                   //RETURN TO USER: MEASURED VOLTAGE or CURRENT
       ...
       if(head->type ==0) {
           ...
           if(onArray[head->channel-1] == 1) {             //means can send out to FPGA
               ...
               send = send << 0x05;                    //shift up 5 spots reset and sweep are 0!
               val = head->channel-1;
               send |= (val & 0x4);
               send |= (val & 0x2);
               send |= (val & 0x1);
               send = send << 0x01;                 //TODO CHANGING THIS !!!
   ...
   //     print_Send(send);   //send this to FPGA
        ...
        val=0;
        val = head->stepsize;
        val <<= 0x1d;                               //need to add step size -- (now default is 0.5 but will need to add this functionality.
        send |= val;
        val =0;
        val = head->frequency;
        val <<= 0x029;                               //need to add step size -- (now default is 0.5 but will need to add this functionality.
        send |= val;
        send = send;
